...
def parse_job_data(response_data: dict):
    ...
    try:
        ...
        for element in elements:
            ...
            job_card = element.get('jobCard', {})

            # Extract the title and ID
            # title_view_model = job_card.get('jobPostingTitle', {})
            job_posting_card_wrapper = job_card.get('jobPostingCardWrapper', {})
            job_tracking_data = job_posting_card_wrapper.get('jobTrackingData', {})
            navigation_action = job_tracking_data.get('navigationAction', {})
            job_url = navigation_action.get('actionTarget', None)
            if job_url:
...
if __name__ == "__main__":
    # --- Example Usage ---
    search_keywords = ["Spring Boot", "java", "python", "devops"]
    job_count = 10  # Let's fetch 5 jobs for this example
    start_index = 0

    print(
        f"Searching for {job_count} jobs with keywords: {', '.join(search_keywords)}, starting from index {start_index}.")

    # 1. Get the raw job data from the API
    raw_job_results = get_linkedin_jobs(search_keywords, count=job_count, start=start_index)

    # 2. Parse the raw data to get a clean list
    if raw_job_results and "error" not in raw_job_results:
        parsed_jobs = parse_job_data(raw_job_results)

        parsed_jobs = parse_job_id_from_urls(parsed_jobs)

        for parsed_job_id in parsed_jobs:
            res = get_linkedin_job_details(parsed_job_id)
            print(f"{parsed_job_id} : ")
            print(parse_job_json_response(res))

        print("\n--- Parsed Job Listings ---")
        if parsed_jobs:
            print(json.dumps(parsed_jobs, indent=2))
        else:
            print("No job listings were found or parsed.")
    else:
        print("\n--- API Response ---")
        print(json.dumps(raw_job_results, indent=2))
